# Run Record: workflow-audit-post-L2-nPPI-ecn-20260903

- Timestamp: 20260903-185842
- Branch: main
- Commit: 2b9d701
- Host: CLA19787.tu.temple.edu
- User: tug87422
- Working directory: `/ZPOOL/data/projects/rf1-sra-ugr`
- Raw log: `/ZPOOL/data/projects/rf1-sra-ugr/logs/runs/20260903-185842_workflow-audit-post-L2-nPPI-ecn-20260903.log`
- Command exit: 0
- Check exit: none
- Summary: COMMAND COMPLETED: no check command provided.

## Command

```bash
python3 code/audit_workflow.py --sessions all --ppi-type nppi-ecn --fsl-root /ZPOOL/data/projects/rf1-sra-ugr/derivatives/fsl --output-dir logs/audits/20260903-post-L2-nppi-ecn
```

## Full Log

```text
RUN START: 20260903-185842
PROJECT_ROOT: /ZPOOL/data/projects/rf1-sra-ugr
GIT: main 2b9d701
HOST: CLA19787.tu.temple.edu
USER: tug87422
PWD: /ZPOOL/data/projects/rf1-sra-ugr
COMMAND: python3 code/audit_workflow.py --sessions all --ppi-type nppi-ecn --fsl-root /ZPOOL/data/projects/rf1-sra-ugr/derivatives/fsl --output-dir logs/audits/20260903-post-L2-nppi-ecn

UGR WORKFLOW AUDIT
BIDS root: /ZPOOL/data/projects/rf1-sra-linux2/bids
fMRIPrep root: /ZPOOL/data/projects/rf1-sra-linux2/derivatives/fmriprep
Confounds root: /ZPOOL/data/projects/rf1-sra-linux2/derivatives/fsl/confounds_tedana
FSL derivatives root: /ZPOOL/data/projects/rf1-sra-ugr/derivatives/fsl
Sessions audited: 01,02
Visible UGR units: 713
Input-ready units: 712
Input-missing units: 1
EV-complete units: 711
EV todo: 1
L1 activation complete: 711
L1 activation todo: 0
L1 activation blocked by EVs: 1
L1 nppi-ecn complete: 711
L1 nppi-ecn todo: 0
L1 nppi-ecn blocked by EVs/activation: 1
Input-ready subject-sessions with runs 1 and 2: 342
L2 activation complete/eligible: 341/341
L2 activation todo: 0
L2 activation blocked by L1: 1
L2 nppi-ecn complete/eligible: 341/341
L2 nppi-ecn todo: 0
L2 nppi-ecn blocked by L1: 1
Detailed reports: /ZPOOL/data/projects/rf1-sra-ugr/logs/audits/20260903-post-L2-nppi-ecn

COMMAND EXIT: 0
```
