...
- **Purpose:** Validate one subject/session/run, render activation or seed-PPI model 3, and optionally run FEAT.
- **Inputs:** Canonical BOLD, confounds, model-3 EVs, FEAT template, and optional `masks/seed-<name>.nii.gz`.
- **Outputs:** Rendered FSF, L1 FEAT, and PPI physiological series when requested.
...
- **Scientific role:** Runs the established 11-EV/17-contrast activation model or betrayal-derived seed PPI.
- **Important assumptions:** Activation must exist before PPI. Already-normalized input uses identity registration links after FEAT.

## `run_L1stats.sh`

- **Status:** Production batch wrapper.
- **Purpose:** Run manifest units with bounded deterministic shell job control and per-unit logs.
...
- **Inputs:** FSL derivative root, sessions, optional subject list, and `act` or `ppi_seed-<seed>`.
